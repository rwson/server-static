...
try {
    ...
    if (outConfig.ignores) {
        if (Array.isArray(outConfig.ignores) && outConfig.ignores.every(fn => typeof arr === "function")) {
            ignores = ignores.concat(outConfig.ignores);
        } else if (typeof outConfig.ignores === "function") {
            ignores.push(outConfig.ignores);
        } else {
            throw "ignores should be function or an array of functions with return value!";
        }
    }
